...
def plot_timings(experiment, Ns, kernel, fname_out, kernel_map_cpu={}, kernel_map_gpu={}):
    plt.clf()
    data_cpu, data_gpu = read_kernel_timings(experiment, Ns, kernel, kernel_map_cpu, kernel_map_gpu)
    plt.loglog(Ns, [d/1e6 for d in data_cpu], '-o', label='CPU')
    plt.loglog(Ns, [d/1e6 for d in data_gpu], '-o', label='GPU')
    plt.xlabel('N')
    plt.ylabel('t [ms]')
    plt.grid(which='both')
    plt.legend()
...
if __name__ == '__main__':
    ...
    kernel_map_gpu = {'FFT::forward':'CUFFT::forward',
                      ...
                      'computeDudt':'IncompressibleEuler::computeDudt',
                      'pad':'copy_to_padded 3d'}
    plot_timings(args.experiment, args.N, args.kernel, args.file_out, kernel_map_cpu, kernel_map_gpu)

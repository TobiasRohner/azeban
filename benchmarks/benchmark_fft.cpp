...
int main(int argc, char** argv){
  ...
  zisa::array<azeban::complex_t, 4> u_hat_host = grid.make_array_fourier(1, zisa::device_type::cpu, MPI_COMM_WORLD);
  zisa::array<azeban::complex_t, 4> u_hat_device = grid.make_array_fourier(1, zisa::device_type::cuda, MPI_COMM_WORLD);
  zisa::array<azeban::real_t, 4> u_host = grid.make_array_phys(1, zisa::device_type::cpu, MPI_COMM_WORLD);
  zisa::array<azeban::real_t, 4> u_device = grid.make_array_phys(1, zisa::device_type::cuda, MPI_COMM_WORLD);
  ...
  if (rank == 0) {
    std::cout << "Forward:  min=" << min_forward << "s, max=" << max_forward << "s, mean=" << mean_forward << "s\n";
    std::cout << "Backward: min=" << min_backward << "s, max=" << max_backward << "s, mean=" << mean_backward << "s\n";
  }

...
class FFTWFFT final : public FFT<Dim> {
  ...
  static_assert(std::is_same_v<real_t, std::decay_t<decltype(std::declval<fftw_complex>()[0])>>,
		"Real type has the wrong precision for FFTW");
  static_assert(std::is_same_v<std::decay_t<decltype(std::declval<complex_t>()[0])>,
...
public:
  ...
  FFTWFFT(const zisa::array_view<complex_t, dim_v> &u_hat, const zisa::array_view<real_t, dim_v> &u)
    : FFTWFFT(increase_array_view_rank(u_hat), increase_array_view_rank(u)) { }

  FFTWFFT(const zisa::array_view<complex_t, dim_v+1> &u_hat, const zisa::array_view<real_t, dim_v+1> &u)
      : super(u_hat, u) {
    assert(u_hat.memory_location() == zisa::device_type::cpu && "FFTW is CPU only!");
    assert(u.memory_location() == zisa::device_type::cpu && "FFTW is CPU only!");

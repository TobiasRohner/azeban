...
struct Complex {
  ...
  ANY_DEVICE_INLINE Complex(scalar_t real, scalar_t imag=0) : x(real), y(imag) { }
...
template<typename Scalar>
ANY_DEVICE_INLINE Complex<Scalar> operator+(Complex<Scalar> c1, const Complex<Scalar>& c2) {
  return c1 += c2;
}
...
template<typename Scalar>
ANY_DEVICE_INLINE Complex<Scalar> operator-(Scalar real, const Complex<Scalar>& c) {
  return real + (-c);
}

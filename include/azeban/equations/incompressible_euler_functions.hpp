...
ANY_DEVICE_INLINE void incompressible_euler_2d_compute_B(unsigned stride, unsigned idx, real_t norm, real_t u1, real_t u2, real_t *B) {
  B[0 * stride + idx] = norm * u1 * u1;
  B[1 * stride + idx] = norm * u1 * u2;
  B[2 * stride + idx] = norm * u2 * u2;
}

ANY_DEVICE_INLINE void incompressible_euler_3d_compute_B(unsigned stride, unsigned idx, real_t norm, real_t u1, real_t u2, real_t u3, real_t *B) {
  B[0 * stride + idx] = norm * u1 * u1;
...
ANY_DEVICE_INLINE void incompressible_euler_2d_compute_L(real_t k1, real_t k2, real_t absk2, unsigned stride_B, unsigned idx_B, const complex_t *B_hat, complex_t *L1_hat, complex_t *L2_hat) {
  ...
  *L1_hat = (1. - (k1 * k1) / absk2) * b1_hat + (0. - (k1 * k2) / absk2) * b2_hat;
  *L2_hat = (0. - (k2 * k1) / absk2) * b1_hat + (1. - (k2 * k2) / absk2) * b2_hat;
}

ANY_DEVICE_INLINE void incompressible_euler_3d_compute_L(real_t k1, real_t k2, real_t k3, real_t absk2, unsigned stride_B, unsigned idx_B, const complex_t *B_hat, complex_t *L1_hat, complex_t *L2_hat, complex_t *L3_hat) {
  const complex_t B11_hat = B_hat[0 * stride_B + idx_B];

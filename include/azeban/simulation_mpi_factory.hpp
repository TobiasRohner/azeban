...
#include <azeban/grid_mpi_factory.hpp>


namespace azeban {

template<int Dim>
Simulation<Dim> make_simulation_mpi(const nlohmann::json &config, MPI_Comm comm) {
  int size;
  ...
  if (size == 1) {
    ...
    auto grid = make_grid_mpi<Dim>(config["grid"], comm);
    ...
    auto equation
	= make_equation_mpi<Dim>(config["equation"], grid, has_tracer);
    ...
    auto timestepper
	= make_timestepper(config["timestepper"], grid, equation, zisa::device_type::cpu);
    ...
    return Simulation(
	grid.shape_fourier(equation->n_vars()), cfl, timestepper, zisa::device_type::cpu);
  }
}

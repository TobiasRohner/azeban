...
public:
  ...
  FFT(const zisa::array_view<complex_t, dim_v> &u_hat,
...
protected:
  ...
  static zisa::array_view<T, D + 1>
  increase_array_view_rank(const zisa::array_view<T, D> &arr) {
    ...
    return zisa::array_view<T, D + 1>(
        shape_new, arr.raw(), arr.memory_location());
  }
};

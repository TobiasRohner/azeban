...
template<typename RNG, typename = typename std::enable_if<RNGTraits<RNG>::location == zisa::device_type::cuda>::type>
...
#define AZEBAN_INSTANTIATE_CURAND_HELPERS(RNG) \
  extern template void curand_allocate_state<RNG>(typename RNGTraits<RNG>::state_t &, size_t, unsigned long long); \
  extern template void curand_free_state<RNG>(typename RNGTraits<RNG>::state_t &);

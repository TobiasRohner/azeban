...
template <typename T>
__global__ void copy_to_padded_cuda_kernel(zisa::array_view<T, 1> dst,
                                           zisa::array_const_view<T, 1> src,
                                           T pad_value) {
...
template <typename T>
__global__ void copy_to_padded_cuda_kernel(zisa::array_view<T, 2> dst,
                                           zisa::array_const_view<T, 2> src,
                                           T pad_value) {
...
template <typename T>
__global__ void copy_to_padded_cuda_kernel(zisa::array_view<T, 3> dst,
                                           zisa::array_const_view<T, 3> src,
                                           T pad_value) {
  const int i = blockIdx.x * blockDim.x + threadIdx.x;
  const int j = blockIdx.y * blockDim.y + threadIdx.y;
...
template <typename T>
__global__ void copy_from_padded_cuda_kernel(zisa::array_view<T, 1> dst,
                                             zisa::array_const_view<T, 1> src) {
  const int idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (idx < dst.shape(0)) {
    dst[idx] = src[idx];
...
template <typename T>
__global__ void copy_from_padded_cuda_kernel(zisa::array_view<T, 2> dst,
                                             zisa::array_const_view<T, 2> src) {
  const int i = blockIdx.x * blockDim.x + threadIdx.x;
  const int j = blockIdx.y * blockDim.y + threadIdx.y;
  const auto src_shape = src.shape();
...
template <typename T>
__global__ void copy_from_padded_cuda_kernel(zisa::array_view<T, 3> dst,
                                             zisa::array_const_view<T, 3> src) {
  const int i = blockIdx.x * blockDim.x + threadIdx.x;
  const int j = blockIdx.y * blockDim.y + threadIdx.y;
  const int k = blockIdx.z * blockDim.z + threadIdx.z;
...
template <typename T>
void copy_to_padded_cuda(const zisa::array_view<T, 1> &dst,
                         const zisa::array_const_view<T, 1> &src,
                         const T &pad_value) {
...
template <typename T>
void copy_to_padded_cuda(const zisa::array_view<T, 2> &dst,
                         const zisa::array_const_view<T, 2> &src,
                         const T &pad_value) {
  assert(src.memory_location() == zisa::device_type::cuda);
  assert(dst.memory_location() == zisa::device_type::cuda);
  assert(dst.shape(0) >= src.shape(0));
...
template <typename T>
void copy_to_padded_cuda(const zisa::array_view<T, 3> &dst,
                         const zisa::array_const_view<T, 3> &src,
                         const T &pad_value) {
  assert(src.memory_location() == zisa::device_type::cuda);
  assert(dst.memory_location() == zisa::device_type::cuda);
  assert(dst.shape(0) >= src.shape(0));
  ...
  const dim3 block_dims(
      zisa::min(zisa::div_up(static_cast<int>(dst.shape(0)), thread_dims.x), 8),
...
template <typename T>
void copy_from_padded_cuda(const zisa::array_view<T, 1> &dst,
                           const zisa::array_const_view<T, 1> &src) {
  assert(src.memory_location() == zisa::device_type::cuda);
  assert(dst.memory_location() == zisa::device_type::cuda);
  assert(dst.shape(0) <= src.shape(0));
...
template <typename T>
void copy_from_padded_cuda(const zisa::array_view<T, 2> &dst,
                           const zisa::array_const_view<T, 2> &src) {
  assert(src.memory_location() == zisa::device_type::cuda);
  assert(dst.memory_location() == zisa::device_type::cuda);
  assert(dst.shape(0) <= src.shape(0));
...
template <typename T>
void copy_from_padded_cuda(const zisa::array_view<T, 3> &dst,
                           const zisa::array_const_view<T, 3> &src) {
  assert(src.memory_location() == zisa::device_type::cuda);
  assert(dst.memory_location() == zisa::device_type::cuda);
  assert(dst.shape(0) <= src.shape(0));

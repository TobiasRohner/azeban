...
__global__ void norm_cuda_kernel(zisa::array_const_view<Scalar, 1> in_data, zisa::array_view<real_t, 1> out_data, real_t p) {
...
real_t norm_cuda(const zisa::array_const_view<Scalar, 1> &data, real_t p) {
  ...
  int block_dims = zisa::min(zisa::div_up(static_cast<int>(data.shape(0)), 2*thread_dims), 1024);
  auto out_data = zisa::cuda_array<real_t, 1>(zisa::shape_t<1>(block_dims));
  norm_cuda_kernel<<<block_dims, thread_dims, thread_dims*sizeof(real_t)>>>(data,
									    zisa::array_view<real_t, 1>(out_data),
									    p);
  while (block_dims > 1) {
    block_dims = zisa::min(zisa::div_up(static_cast<int>(block_dims), 2*thread_dims), 1024);
    norm_cuda_kernel<<<block_dims, thread_dims, thread_dims*sizeof(real_t)>>>(zisa::array_const_view<real_t, 1>(out_data),
									      zisa::array_view<real_t, 1>(out_data),
									      p);
  }
  zisa::array<real_t, 1> value(zisa::shape_t<1>(1));
  zisa::internal::copy(value.raw(), value.device(),
		       out_data.raw(), out_data.device(),
		       1);
  return value[0];
}

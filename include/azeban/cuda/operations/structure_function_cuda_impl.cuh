...
structure_function_cuda_kernel(const zisa::array_const_view<real_t, 2> u,
			       ...
			       ssize_t stride,
                               ...
                               ssize_t i0) {
  const ssize_t i = stride * (i0 + blockDim.x * blockIdx.x + threadIdx.x);
...
structure_function_cuda_kernel(const zisa::array_const_view<real_t, 3> u,
  ...
  const ssize_t i = stride * (i0 + i_loc);
  const ssize_t j = stride * (j0 + j_loc);
  const ssize_t N = u.shape(1);
  ...
  for (ssize_t di = -max_h + 1; di < max_h; ++di) {
    ...
    for (ssize_t dj = -max_h + 1; dj < max_h; ++dj) {
      ...
      sf[h * h_stride + thread_idx] += func(uij, vij, ukl, vkl, di, dj);
    }
  }
}
...
structure_function_cuda_kernel(const zisa::array_const_view<real_t, 4> u,
			       ...
			       ssize_t stride,
                               Function func,
                               const zisa::array_view<double, 4> sf,
                               ssize_t i0,
                               ssize_t j0,
                               ssize_t k0) {
  ...
  const ssize_t i = stride * (i0 + i_loc);
  const ssize_t j = stride * (j0 + j_loc);
  const ssize_t k = stride * (k0 + k_loc);
  const ssize_t N = u.shape(1);
    ...
    for (ssize_t dj = -max_h + 1; dj < max_h; ++dj) {
      ...
      for (ssize_t dk = -max_h + 1; dk < max_h; ++dk) {
        ...
        sf[h * h_stride + thread_idx]
            += static_cast<double>(func(uijk, vijk, wijk, ulmn, vlmn, wlmn, di, dj, dk));
      }
    }
  }
}
...
structure_function_cuda(const zisa::array_const_view<real_t, 2> &u,
  ...
  const dim3 block_dims(
      zisa::div_up(static_cast<int>(u.shape(1) / stride), thread_dims.x), 1, 1);
...
structure_function_cuda(const zisa::array_const_view<real_t, 3> &u,
  ...
  const dim3 block_dims(
      zisa::div_up(zisa::min(static_cast<int>(u.shape(1) / stride), 128), thread_dims.x),
      zisa::div_up(zisa::min(static_cast<int>(u.shape(2) / stride), 128), thread_dims.y),
      1);
  ...
  zisa::array<real_t, 3> sf(zisa::shape_t<3>(max_h,
                            ...
                            zisa::device_type::cuda);
  zisa::fill(sf.raw(), sf.device(), sf.size(), real_t(0));
  for (ssize_t i0 = 0; i0 < static_cast<ssize_t>(u.shape(1)/stride);
       i0 += grid_dims.y) {
    for (ssize_t j0 = 0; j0 < static_cast<ssize_t>(u.shape(2)/stride);
         j0 += grid_dims.x) {
      structure_function_cuda_kernel<<<block_dims, thread_dims>>>(
          u, max_h, stride, func, sf.view(), i0, j0);
      cudaDeviceSynchronize();
      ZISA_CHECK_CUDA_DEBUG;
    }
  }
  ...
  for (ssize_t h = 0; h < max_h; ++h) {
    ret[h] = reduce_sum(
        zisa::array_view<real_t, 1>(zisa::shape_t<1>(sf.shape(1) * sf.shape(2)),
                                    &sf(h, 0, 0),
                                    zisa::device_type::cuda)) / zisa::pow<2>(u.shape(1) / stride);
  }
  return ret;
}
...
structure_function_cuda(const zisa::array_const_view<real_t, 4> &u,
  ...
  const dim3 block_dims(
      zisa::div_up(zisa::min(static_cast<int>(u.shape(1) / stride), 128), thread_dims.x),
      zisa::div_up(zisa::min(static_cast<int>(u.shape(2) / stride), 128), thread_dims.y),
      zisa::div_up(zisa::min(static_cast<int>(u.shape(3) / stride), 128),
                   thread_dims.z));
  const dim3 grid_dims(thread_dims.x * block_dims.x,
                       thread_dims.y * block_dims.y,
                       thread_dims.z * block_dims.z);
  zisa::array<double, 4> sf(zisa::shape_t<4>(max_h,
                                             thread_dims.z * block_dims.z,
                                             thread_dims.y * block_dims.y,
                                             thread_dims.x * block_dims.x),
                            zisa::device_type::cuda);
  zisa::fill(sf.raw(), sf.device(), sf.size(), double(0));
  for (ssize_t i0 = 0; i0 < static_cast<ssize_t>(u.shape(1)/stride);
       i0 += grid_dims.z) {
    for (ssize_t j0 = 0; j0 < static_cast<ssize_t>(u.shape(2)/stride);
         j0 += grid_dims.y) {
      for (ssize_t k0 = 0; k0 < static_cast<ssize_t>(u.shape(3)/stride);
           k0 += grid_dims.x) {
        structure_function_cuda_kernel<<<block_dims, thread_dims>>>(
            u, max_h, stride, func, sf.view(), i0, j0, k0);
        cudaDeviceSynchronize();
        ZISA_CHECK_CUDA_DEBUG;
      }
    }
  ...
  std::vector<real_t> ret(max_h);
  for (ssize_t h = 0; h < max_h; ++h) {
    ret[h] = reduce_sum(zisa::array_view<double, 1>(
        zisa::shape_t<1>(sf.shape(1) * sf.shape(2) * sf.shape(3)),
        &sf(h, 0, 0, 0),
        zisa::device_type::cuda)) / zisa::pow<3>(u.shape(1)/stride);
  }
  return ret;
}

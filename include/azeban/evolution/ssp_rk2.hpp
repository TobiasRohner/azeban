...
public:
  ...
  SSP_RK2(zisa::device_type device,
      ...
      :	super(device, equation),
	u_star_(shape, device),
	dudt_(shape, device) { }
  ...
  virtual void integrate(real_t dt,
    ...
    zisa::internal::copy(dudt_.raw(), dudt_.device(),
			 u.raw(), u.memory_location(),
			 zisa::product(dudt_.shape()));
    zisa::internal::copy(u_star_.raw(), u_star_.device(),
			 u.raw(), u.memory_location(),
			 zisa::product(dudt_.shape()));
    ...
    axpy(scalar_t(0.5*dt), zisa::array_const_view<scalar_t, dim_v>(u_star_), u);
  }

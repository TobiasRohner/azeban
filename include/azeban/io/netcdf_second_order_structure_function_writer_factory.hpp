...
make_netcdf_second_order_structure_function_writer(
    int ncid,
    const nlohmann::json &config,
    const Grid<Dim> &grid,
    zisa::int_t sample_idx_start) {
  if (!config.contains("snapshots")) {
    LOG_ERR("NetCDFSecondOrderStructureFunctionWriter config needs key "
            "\"snapshots\"");
  }
  ...
  if (exact) {
    return std::make_unique<
        NetCDFSecondOrderStructureFunctionWriter<Dim, detail::SF2Exact>>(
        ncid, grid, snapshots, sample_idx_start);
  } else {
    return std::make_unique<
        NetCDFSecondOrderStructureFunctionWriter<Dim, detail::SF2Approx>>(
        ncid, grid, snapshots, sample_idx_start);
  }
}

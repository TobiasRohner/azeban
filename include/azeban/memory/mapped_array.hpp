...
template <typename T, int n_dims>
zisa::array_view<T, n_dims>
mapped_array_to_cuda_view(const zisa::array_view<T, n_dims> &arr) {
  T *ptr;
...
mapped_array_to_cuda_view(const zisa::array_const_view<T, n_dims> &arr) {
  T *ptr;
  cudaHostGetDevicePointer(&ptr, arr.raw(), 0);
  return zisa::array_const_view<T, n_dims>(
      arr.shape(), ptr, zisa::device_type::cuda);
}

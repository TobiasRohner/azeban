...
make_second_order_structure_function_writer_type(const nlohmann::json &config,
                                                 const Grid<Dim> &grid,
                                                 zisa::int_t sample_idx_start) {
  if (!config.contains("path")) {
    fmt::print(
        stderr,
        "SecondOrderStructureFunctionWriter config needs key \"path\"\n");
    exit(1);
  }
  if (!config.contains("snapshots")) {
    fmt::print(
        stderr,
        "SecondOrderStructureFunctionWriter config needs key \"snapshots\"\n");
    exit(1);
  }
...
make_second_order_structure_function_writer(const nlohmann::json &config,
                                            const Grid<Dim> &grid,
                                            zisa::int_t sample_idx_start) {
  bool exact = false;
  ...
  if (exact) {
    return make_second_order_structure_function_writer_type<
        Dim,
        SecondOrderStructureFunctionWriter2Exact<Dim>>(
        config, grid, sample_idx_start);
  } else {
    return make_second_order_structure_function_writer_type<
        Dim,
        SecondOrderStructureFunctionWriter2Approx<Dim>>(
        config, grid, sample_idx_start);
  }
}
...
make_second_order_structure_function_writer<1>(const nlohmann::json &,
                                               const Grid<1> &,
                                               zisa::int_t);
template std::unique_ptr<Writer<2>>
make_second_order_structure_function_writer<2>(const nlohmann::json &,
                                               const Grid<2> &,
                                               zisa::int_t);
template std::unique_ptr<Writer<3>>
make_second_order_structure_function_writer<3>(const nlohmann::json &,
                                               const Grid<3> &,
                                               zisa::int_t);
#if AZEBAN_HAS_MPI
template std::unique_ptr<Writer<1>>

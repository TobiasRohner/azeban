...
template <int Dim>
NetCDFEnergySpectrumWriter<Dim>::NetCDFEnergySpectrumWriter(
    int ncid,
    const Grid<Dim> &grid,
  ...
  // Define own group
  CHECK_NETCDF(nc_def_grp(ncid_, "energy_spectrum", &grpid_));
  // Define dimensions
  int dimid_member;
  int dimid_time;
  int dimid_k;
...
template class NetCDFEnergySpectrumWriter<1>;
template class NetCDFEnergySpectrumWriter<2>;
template class NetCDFEnergySpectrumWriter<3>;

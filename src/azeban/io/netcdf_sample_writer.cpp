...
NetCDFSampleWriter<Dim>::NetCDFSampleWriter(
    ...
    : super(ncid, grid, snapshot_times, sample_idx_start), N_(N) {
  // Define own group
  CHECK_NETCDF(
  ...
  if (N_ != grid_.N_phys) {
    ...
    u_hat_down_ = grid_down.make_array_fourier(Dim, zisa::device_type::cpu);
    u_down_ = grid_down.make_array_phys(Dim, zisa::device_type::cpu);

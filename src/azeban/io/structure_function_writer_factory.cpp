...
make_structure_function_writer_type(const nlohmann::json &config,
  ...
  if (!config.contains("snapshots")) {
    fmt::print(stderr,
               "StructureFunctionWriter config needs key \"snapshots\"\n");
    exit(1);
  }
...
make_structure_function_writer(const nlohmann::json &config,
  ...
  if (exact) {
    return make_structure_function_writer_type<
        Dim,
        StructureFunctionWriterExact<Dim>>(config, grid, sample_idx_start);
  } else {
    return make_structure_function_writer_type<
        Dim,
        StructureFunctionWriterApprox<Dim>>(config, grid, sample_idx_start);
  }
}
...
make_structure_function_writer(const nlohmann::json &config,
  ...
  if (exact) {
    return make_structure_function_writer_type<
        Dim,
        StructureFunctionWriterExact<Dim>>(
        config, grid, sample_idx_start, comm);
  } else {
    return make_structure_function_writer_type<
        Dim,
        StructureFunctionWriterApprox<Dim>>(
        config, grid, sample_idx_start, comm);
  }
}
#endif

template std::unique_ptr<Writer<1>> make_structure_function_writer<1>(
    const nlohmann::json &, const Grid<1> &, zisa::int_t);
template std::unique_ptr<Writer<2>> make_structure_function_writer<2>(
    const nlohmann::json &, const Grid<2> &, zisa::int_t);
template std::unique_ptr<Writer<3>> make_structure_function_writer<3>(

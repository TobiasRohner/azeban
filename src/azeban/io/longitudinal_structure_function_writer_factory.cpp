...
make_longitudinal_structure_function_writer(const nlohmann::json &config,
                                            const Grid<Dim> &grid,
                                            zisa::int_t sample_idx_start) {
  if (!config.contains("path")) {
    fmt::print(
        stderr,
        "LongitudinalStructureFunctionWriter config needs key \"path\"\n");
    exit(1);
  }
  if (!config.contains("snapshots")) {
    fmt::print(
        stderr,
        "LongitudinalStructureFunctionWriter config needs key \"snapshots\"\n");
    exit(1);
  }
  if (!config.contains("p")) {
    fmt::print(stderr,
               "LongitudinalStructureFunctionWriter config needs key \"p\"\n");
    exit(1);
  }
...
template std::unique_ptr<Writer<2>> make_longitudinal_structure_function_writer(
    const nlohmann::json &, const Grid<2> &, zisa::int_t);
template std::unique_ptr<Writer<3>> make_longitudinal_structure_function_writer(

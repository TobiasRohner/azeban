...
void DiscontinuousShearTube::do_initialize(const zisa::array_view<real_t, 4> &u) {
  const auto init = [&](auto &&u_) {
    ...
    for (zisa::int_t i = 0; i < N; ++i) {
      ...
      for (zisa::int_t k = 0; k < N_; ++k) {
        sigma_y += alpha_y[k] * zisa::sin(2 * zisa::pi * (k + 1) * x + beta_y[k]);
        sigma_z += alpha_z[k] * zisa::sin(2 * zisa::pi * (k + 1) * x + beta_z[k]);
      }
...
void DiscontinuousShearTube::do_initialize(const zisa::array_view<complex_t, 4> &u_hat) {

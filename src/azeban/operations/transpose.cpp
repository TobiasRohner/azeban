...
void transpose(const zisa::array_view<complex_t, 4> &dst, const zisa::array_const_view<complex_t, 4> &src, MPI_Comm comm) {
  ...
  for (int r = 0 ; r < size ; ++r) {
    ...
    const zisa::shape_t<4> view_shape(ndim, N_loc[2 * r + 1], Nx, N_loc[2 * rank + 0]);
    zisa::array_view<complex_t, 4> view(view_shape, sendbuf.get() + sdispls[r], zisa::device_type::cpu);
  ...
  for (int r = 0 ; r < size ; ++r) {
    ...
    const zisa::shape_t<4> view_shape(ndim, N_loc[2 * rank + 1], Nx, N_loc[2 * r + 0]);
    zisa::array_view<complex_t, 4> view(view_shape, recvbuf.get() + rdispls[r], zisa::device_type::cpu);
    #pragma omp parallel for collapse(4)
    for (zisa::int_t d = 0 ; d < ndim ; ++d) {
      for (zisa::int_t i = 0 ; i < i_end ; ++i) {

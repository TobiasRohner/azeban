...
int main() {
  ...
  CFL cfl(0.5);
  auto equation = std::make_shared<Burgers<SmoothCutoff1D>>(N_phys, SmoothCutoff1D(0./N_phys, 1), zisa::device_type::cuda);
  ...
  for (int i = 0 ; i < 1000 ; ++i) {
    ...
    zisa::internal::copy(u_hat_device.raw(), u_hat_device.device(),
			 ...
			 N_fourier);
    */
    zisa::copy(u_hat_device, zisa::array_const_view<complex_t, 1>(simulation.u()));
    fft->backward();

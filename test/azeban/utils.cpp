		...
		for (zisa::int_t i = 0 ; i < N ; ++i) {
		  for (zisa::int_t j = 0 ; j < N ; ++j) {
		    for (zisa::int_t k = 0 ; k < N ; ++k) {
		      const azeban::real_t x = static_cast<azeban::real_t>(i) / N - 0.5;
		      const azeban::real_t y = static_cast<azeban::real_t>(j) / N - 0.5;
		      const azeban::real_t z = static_cast<azeban::real_t>(k) / N - 0.5;
		      u(0, i, j, k) = 1;
		      u(0, i, j, k) *= zisa::exp(-zisa::pow<2>((x - xbar) / sigmax));
		      u(0, i, j, k) *= zisa::exp(-zisa::pow<2>((y - ybar) / sigmay));
		      u(0, i, j, k) *= zisa::exp(-zisa::pow<2>((z - zbar) / sigmaz));
		    }
		  }
		}
		fft->forward();
		const azeban::real_t norm = L2<3>(u_hat, u_ref_hat);
		azeban::real_t exact = zisa::sqrt(zisa::pow<3>(zisa::pi) / 2) * sigmax * sigmay * sigmaz / 16;
		exact *= std::erf(zisa::sqrt(2) / sigmax * (0.5 - xbar)) - std::erf(zisa::sqrt(2) / sigmax * (-0.5 - xbar));
		exact *= std::erf(zisa::sqrt(2) / sigmay * (0.5 - ybar)) - std::erf(zisa::sqrt(2) / sigmay * (-0.5 - ybar));
		exact *= std::erf(zisa::sqrt(2) / sigmaz * (0.5 - zbar)) - std::erf(zisa::sqrt(2) / sigmaz * (-0.5 - zbar));
		exact = zisa::sqrt(exact);
		REQUIRE(std::fabs(norm - exact) <= 1e-4);
	      }
	    }

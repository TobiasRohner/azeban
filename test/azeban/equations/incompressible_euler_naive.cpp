...
TEST_CASE("2D Euler Naive Derivative") {
  ...
  azeban::IncompressibleEulerNaive<2, azeban::Step1D> euler(
...
TEST_CASE("Euler Naive Taylor Vortex 2D", "[slow]") {
  ...
  for (zisa::int_t N = 16; N < 128; N <<= 1) {
    ...
    azeban::SmoothCutoff1D visc(0.05 / N, 1, 1);
    const auto equation = std::make_shared<
        azeban::IncompressibleEulerNaive<2, azeban::SmoothCutoff1D>>(
        grid, visc, zisa::device_type::cuda);
...
TEST_CASE("Euler Naive Taylor Vortex 3D", "[slow]") {
  ...
  const auto test = [&](int dim) {
    ...
    for (zisa::int_t N = 16; N <= 128; N <<= 1) {
      ...
      azeban::SmoothCutoff1D visc(0.05 / N, 1, 1);
      const auto equation = std::make_shared<
          azeban::IncompressibleEulerNaive<3, azeban::SmoothCutoff1D>>(
          grid, visc, zisa::device_type::cuda);
...
TEST_CASE("Euler Naive Double Shear Layer 3D const. x", "[slow]") {
...
TEST_CASE("Euler Naive Double Shear Layer 3D const. y", "[slow]") {
  const auto initializer2d = std::make_shared<azeban::DoubleShearLayer>(
      azeban::RandomVariable<azeban::real_t>(
          std::make_shared<azeban::Delta<azeban::real_t>>(0.2)),
      azeban::RandomVariable<azeban::real_t>(
...
TEST_CASE("Euler Naive Double Shear Layer 3D const. z", "[slow]") {
  const auto initializer2d = std::make_shared<azeban::DoubleShearLayer>(
      azeban::RandomVariable<azeban::real_t>(
          std::make_shared<azeban::Delta<azeban::real_t>>(0.2)),
      azeban::RandomVariable<azeban::real_t>(
...
TEST_CASE("Euler Naive Taylor Green 2D", "[slow]") {

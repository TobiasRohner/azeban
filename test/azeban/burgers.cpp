...
TEST_CASE("Burgers Convergence") {
  const auto solve_burgers = [&](const zisa::array_view<azeban::real_t, 1> &h_u, azeban::real_t visc, azeban::real_t t) {
    auto d_u = zisa::cuda_array<azeban::real_t, 1>(h_u.shape());
    auto d_u_hat = zisa::cuda_array<azeban::complex_t, 1>(zisa::shape_t<1>(h_u.shape(0)/2+1));

    auto fft = azeban::make_fft<1>(zisa::array_view<azeban::complex_t, 1>(d_u_hat),
    ...
    azeban::CFL cfl(0.1);
    auto equation = std::make_shared<azeban::Burgers<azeban::SmoothCutoff1D>>(h_u.shape(0),
									      azeban::SmoothCutoff1D(visc, 1),
									      zisa::device_type::cuda);
    auto timestepper = std::make_shared<azeban::SSP_RK2<azeban::complex_t, 1>>(zisa::device_type::cuda,
								               d_u_hat.shape(),
								               equation);
    auto simulation = azeban::Simulation<azeban::complex_t, 1>(zisa::array_const_view<azeban::complex_t, 1>(d_u_hat),
							       cfl, timestepper);
  ...
  const auto compute_error = [&](const zisa::array_const_view<azeban::real_t, 1> &u_ref,
  ...
  const azeban::real_t conv_rate = (zisa::log(err[0]) - zisa::log(err[err.size()-1])) / zisa::log(n[n.size()-1]/n[0]);
